...
| **hunter** | Claude Code / Codex CLI が読み取り専用で候補 finding を広めに集める | `run-plan.json`, `pr.diff`, `pr.diff.ranges.txt`, shallow clones | `claude-review.md`, `codex-review.md`, `findings.candidates.json` | hunter timeout/非ゼロ/`PR_DIFF_UNAVAILABLE`/candidate validation 失敗は `failed_stage=hunter` |
| **verifier** | 候補を正規化し、4軸 + evidence ladder + counterexample で絞る。posting policy もここで焼き付ける | `findings.candidates.json`, raw reviews, `metadata.json`, `pr.diff.ranges.txt`, `schemas/findings.v1.json` | `findings.verified.json`, `validation-report.json` | 4軸 gate、range gate、fingerprint gate、Must Fix 件数 gate、同梱 validator 失敗は `failed_stage=verifier` |
