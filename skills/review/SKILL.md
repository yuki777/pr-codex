...
Codex CLI 側のレビュー実行は、本スキル内で `-m gpt-6-astra` と `model_reasoning_effort="max"` を指定して実行する。
...
対象 PR の GitHub Actions / status checks を read-only で取得し、`ci-status.json` と `ci-summary.md` を生成する。ここでは GitHub への write、workflow rerun、cancel は行わない。また、この Step では `status.json` を `running` に更新しない（`running` の書き込みは Step 3 で選定確定直後に実施済み）。`$target_mode == "auto"` では Step 2b の CI success gate で既に生成済みでも、review hunter を起動する直前に再取得して current head の状態を確認する。再取得した `ci-status.json.state` が `success` ではなくなっていた場合、この PR のレビューは中止する: `status.json` は Step 3 で既に `running` になっているため、そのまま放置すると次候補側の周回で 30 分の stale 回収を待つことになる。`date -u +%Y-%m-%dT%H:%M:%S+00:00` で `$finished_at` を取得してから Step 5 の failed 更新（`$failed_stage=ranker`）を実行して `running` を必ず閉じ、Step 2 の次候補へ戻る（`state=failed` は次回の /loop で CI が `success` に戻り次第、自動的に再試行される。#127）。`$target_mode == "direct"` では CI success gate としては扱わず、`failure` / `pending` も reviewer へ渡す context として記録してレビューを継続できる。`statusCheckRollup` が使えない古い `gh` 環境では、同じ `ci_status.py` に REST の `pulls/{number}` と checks/status endpoint の JSON を渡す fallback を使う。
...
- 次アクション: `ci-status.json.state` を review/send/developer bridge の判断材料として保持する。`$target_mode == "auto"` で `success` 以外なら候補スキップとし、`date -u` で `$finished_at` を取得してから Step 5 の failed 更新（`$failed_stage=ranker`）で `running` を閉じ、Step 2 の次候補へ戻る（#127）。`$target_mode == "direct"` なら `failure` / `pending` を reviewer へコンテキストとして渡す。この step 自体で投稿や rerun はしない。このテンプレート（または下の旧 `gh` fallback）自体が非ゼロ終了した場合（`gh api` / `gh pr view` / `ci_status.py` / policy `jq -e` の失敗。CI artifact 再取得失敗）は、`date -u +%Y-%m-%dT%H:%M:%S+00:00` で `$finished_at` を取得してから Step 5 の failed 更新（`$failed_stage=ranker`）を実行して Step 3 で書いた `running` を必ず閉じ、その回は終了する（#127）
...
- `--model "$claude_model"` — Step 3 で `claude-opus-5-5` に固定した `$claude_model`（`metadata.json.review_engines` に記録した Claude モデル ID）と同じ実値に置換して hunter の実行モデルを明示固定する。記録値と実行モデルの一致を構成的に保証し、CLI 既定モデルや環境変数（`ANTHROPIC_MODEL` 等）由来のドリフトを防ぐ（#124）。値が無効なら hunter は CLI エラーで即失敗し、Step 5 の failed 更新へ遷移する（誤った記録のまま投稿しない fail-closed）
...
- `-m gpt-6-astra` — Codex CLI の hunter を GPT-6 Astra に固定する。global flag のため `exec` の前に置く。モデルを変更する場合は Step 3 の `review_engines`（投稿フッター用の記録。#124）も併せて更新する
- `-c 'model_reasoning_effort="max"'` — GPT-6 Astra hunter の reasoning effort を最大値 `max` に明示固定する（#124。Claude hunter の `--effort max` に対応し、Step 3 の `review_engines` の記録値と一致させる）。`--ignore-user-config` により user config は読まないため、固定しない場合は CLI 既定値で実行されてしまい、投稿フッターの表示と実行構成が乖離し得る。global flag のため `exec` の前に置く（`codex-cli 0.146.0` + `gpt-6-astra` で `reasoning effort: max` の実行を確認済み）
...
```
   ...
   - controller 呼び出しのたびに、`refinement-plan.json.candidate_updates[]` を Read し、各 `candidate_id` と一致する `findings.candidates.json.tmp.candidates[]` へ `decision` / `posting` を**一字一句そのまま**反映してから、手順 14 と同じ `validate_candidates.py` を実行して終了コード 0 を確認する。これはモデル判断ではなく controller-owned transition（controller が決める状態遷移）である。各 update は最新 round の `rejected_candidates[]` に `reason="insufficient_evidence"` / `local_only=true` として 1 回だけ記録する。反映と検証が終わる前に halt 判定を採用してはならない。その後、`refinement-plan.json.should_run == false` なら `halting.reason` を採用して停止する。`should_run == true` の場合だけ、`target_candidate_ids[]` に列挙された候補だけを、その順序を保って `refine` → `challenge` → `verify` する。round 1 は全候補、round 2 は未解決のうち high-risk（Must Fix / security / runtime error / systemic blast radius / security・data migration PR）、hunter 間不一致、または `evidence_state=needs_evidence` の候補、round 3 は **round 2 で host-owned state が変化した high-risk 候補だけ**とする。後続 round で対象を再拡大してはならない。
...
```
   ...
   - **verifier-owned terminal state**: hunter 合成直後は `decision` を持たない ACTIVE state とする。verifier がその round で候補を解決した時点で、canonical finding として採用する場合は `decision="verified"`、反証成立なら `decision="refuted"`、根拠不足または local-only 抑止なら `decision="suppressed"` を設定する。最終 round より前に追加検証が必要な候補だけは `decision` 未設定の ACTIVE state を維持してよい。round 終了後に次 round の選別条件を満たさない候補、および hard halt（`max_rounds` / `time_budget` / `no_new_evidence` / `repeated_contradiction`）時点で残る未解決候補は、次の controller 結果の `candidate_updates[]` が `decision="suppressed"` / `posting.post_policy="local_only"` へ遷移させる。したがって停止後に ACTIVE candidate を残してはならない。最終 round では verifier が全対象を terminal state に遷移させ、controller の hard halt 更新を安全網として適用する。`disagreement` / `severity_disputed` / `contradiction` を保持する場合は boolean に限定する。これらは `findings.candidates.v1` の正式 field であり、任意文字列や別名を追加してはならない。round state を `findings.candidates.json.tmp` へ書き戻した直後は、次の controller 呼び出しより前に手順 14 と同じ `validate_candidates.py` コマンドを実行し、終了コード 0 を確認する。
   - round 終了ごとに、現在の host-owned candidate state を `findings.candidates.json.tmp` へ、round 結果を `refinement-state.json.rounds[]` へ Write する。次の controller 呼び出し後、`candidate_updates[]` を候補 state へ反映したうえで、返された `round_state.state_digest_before` / `state_digest_after` / `changed_candidate_ids` / `changed_candidate_count` / `evidence_added_count` / `disposition_changed_count` / `untargeted_candidate_ids` / `untargeted_candidate_count` / `remaining_active_count` を最新 round へ**一字一句そのまま**反映する。これらをモデル申告値から作らない。同一 digest なら、モデルが新規根拠ありと申告しても controller は `no_new_evidence` で停止する。
   - controller が `round_state.halt_basis_state_digest_after` を返した場合も最新 round へそのまま反映する。これは terminal suppression 適用前に halt 判定へ使った host-owned digest であり、`state_digest_after` は suppression 適用後の最終 candidate state を表す。両者を同じ値に上書きしてはならない。これにより `no_new_evidence` の判定根拠を保持したまま、停止後の全候補を terminal state として記録できる。
   - `round_state.untargeted_candidate_ids[]` と `untargeted_candidate_count` も同じ host-owned field として最新 round へそのまま反映し、`untargeted_candidate_count == len(untargeted_candidate_ids)` を守る。これらは、次 round の priority 対象外、または terminal halt により controller が ACTIVE から `suppressed` へ遷移させた候補を表す。0 より大きく、より優先される hard halt がない場合の halt reason は `no_active_candidates` とする。過去 round の合計値を使ったり `all_candidates_verified` として扱ってはならない。
   ...
   - round 1 は全候補の terminal state と監査記録を作るため必須であり、`time_budget_ms` は round 1 完了後に追加 round を開始できるかを制御する。したがって `time_budget_ms=0` は「round 1 を省略する」ではなく「追加 round を実行しない」を意味する。これにより time budget halt にも必ず直前 round が存在し、controller の `candidate_updates[]` / `untargeted_candidate_ids[]` / local-only rejection をその round へ記録できる。
   ...
   - **review-rounds カウンタ定義**: `input_candidates_count` は round の `target_candidate_ids[]` 件数、`output_candidates_count` は round 終了後も loop に残る ACTIVE 候補数であり、controller の `round_state.remaining_active_count` と一致させる。`changed_candidate_count == len(changed_candidate_ids)`、`untargeted_candidate_count == len(untargeted_candidate_ids)`、`evidence_added_count <= changed_candidate_count`、`disposition_changed_count <= changed_candidate_count` を守る。`suppressed_candidate_count` は canonical findings の件数ではなく `local_only=true` の rejected candidates 件数であり、controller の `candidate_updates[]` による抑止も含める。`verifier_fail_candidates` は `reason="verifier_fail"` の件数。
...
5. ループ通過後、verifier が candidates を絞り込み、**`findings.verified.json` をメモリ上で構築する**。`findings.verified.json` は `schemas/findings.v1.json` に従い、最低限以下を満たす:
   ...
   - `findings[]` へ採用できるのは `decision="verified"` の candidate だけとする。`decision` 未設定の ACTIVE candidate、`refuted`、`suppressed` は canonical findings / `review.md` / GitHub 投稿から除外し、`review-rounds.json.rejected_candidates[]` または `untargeted_candidate_count` による local-only 記録だけを残す。
...
- いつ使うか: `$head_sha` 取得済みの状態で `state=failed` を記録するすべての失敗で実行する。Step 2b の files 取得失敗、Step 3 の `gh pr diff` 失敗、Step 3a の CI artifact 再取得失敗・再取得後の非 success 中止・clone / fetch / checkout 失敗、`run-plan.json` 生成失敗、Step 4a または 4b が timeout / 非ゼロ終了した場合、権限不足などで処理継続不可の場合、**Step 4c の `merge_hunter_results.py` が `HUNTER_DIFF_UNAVAILABLE`（終了コード 3）を返した場合、もしくは hunter result の schema 不適合が 1 回の再実行でも解消しなかった場合**を含む（#127）。`$head_sha` が未取得の場合は、代わりに下の `head_sha` キー省略テンプレートを使う
