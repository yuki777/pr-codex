...
- Codex CLI 側のレビュー (hunter) は、スキル内で `-m gpt-5.5` を指定して実行する。send Step 4.5 の semantic preflight は #110 の担当替えに従い `-m gpt-5.6-sol` (GPT-5.6) を指定し、positive fixture の high / xhigh 比較で品質 gate が同値かつ実行時間・token 数が小さかった `model_reasoning_effort="high"` に固定する。hunter は user config から切り離した Codex CLI default のままにする。hunter (review Step 4b) と投稿前検証 (send Step 4.5) はどちらも `--ignore-user-config` を使うため (#111)、user config 由来の外部 MCP は無効。GitHub 由来のレビュー文脈（PR 説明文・既存レビューコメント）は、親（メインコンテキスト）が read-only で取得した `pr-context.md`（sanitized context pack）として hunter に渡す（外部への到達経路の遮断は、4a が `--setting-sources ""` + `--tools "Read,Glob,Grep,Bash"` + `--strict-mcp-config`、4b が `--ignore-user-config`）
...
- Issue #112 の positive fixture 実測では、GPT-5.6 verifier の exact / acceptable / false-positive / recall gate を維持したまま、3 round (86,460 ms / 66,327 tokens) から controller 停止後の 1 round (43,452 ms / 23,285 tokens) へ削減できた。semantic preflight も xhigh と high が同じ PASS で、high は 32,205 ms / 21,091 tokens から 14,173 ms / 19,690 tokens へ減少した。Fable 5 は baseline prompt の acceptable 0.3333 / false-positive 0.0 から、focused prompt で acceptable 1.0 / false-positive 0.0 へ改善した（35,848 ms → 46,869 ms、token 値は provider 未報告）。再現可能な集計は `fixtures/positive/eval-report.json` に保存する
