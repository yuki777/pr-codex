...
- Claude Code 側のレビュー (hunter) は、review Step 3 で `claude-opus-5-5` に固定した値を `--model` で明示指定して実行し、CLI 既定モデルや環境変数由来のドリフトを防ぐ（#124）。投稿フッターに表示するモデル名は固定値ではなく、review Step 4c が実行証跡（Claude CLI result wrapper (`claude-review.result.json`) の `modelUsage` と `codex.log` の `model:` 行）から `metadata.json.review_engines[].model` へ転記した、実際に使用されたモデル名を使う。転記に失敗した場合はレビューを failed とし、実行事実と一致しないフッターでは投稿しない（#143）
